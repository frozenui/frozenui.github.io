...
(function($) {
   //ajax方法
    ...
    $.ajax({
        type: "GET",
        url: "http://ryanbay.cn/vipstyle/web/case/getCategory.json",
        dataType: 'json',
        timeout: 9999999
    })
    .done(function(data) {
        ...
        for(var i=0; i<j; i++){
            $("#menu").append('<li><a href="#">'+category[i].name+'</a></li>');
        }
    })
    .fail(function() {
        alert('获取失败，请刷新重试。');
    ...
    function getData(name,num){
      ...
      $.ajax({
         type: "GET",
         url: "http://ryanbay.cn/vipstyle/web/case/getData.json",
         dataType: 'json',
         timeout: 9999999
      })
    .done(function(data) {
        if(data.length == 1 && JSON.stringify(data[0]).indexOf("thumb") == -1){  //没有数据
            ...
            for(var i=0; i<j; i++){
               ...
               if(datalist[i].imgPath == undefined){
                  datalist[i].imgPath ='http://ryanbay.cn/vipstyle/web/case/img/';
               }
